...
static inline bool _packet_is_broadcast(gnrc_pktsnip_t* pkt)
{
    ...
    return ( (netif_hdr == NULL) ? false :
                              ((netif_hdr->flags & GNRC_NETIF_HDR_FLAGS_BROADCAST)|(netif_hdr->flags & GNRC_NETIF_HDR_FLAGS_MULTICAST)) );
}
...
void iqueuemac_broadcast_receive_packet_process(iqueuemac_t* iqueuemac);
