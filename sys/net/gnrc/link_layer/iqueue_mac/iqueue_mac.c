...
void iqueuemac_init(iqueuemac_t* iqueuemac)
{
	...
	if(iqueuemac->mac_type == ROUTER)
	{
		...
		iqueuemac->router_states.router_t2r_state = R_T2R_WAIT_CP_INIT;
		iqueuemac->router_states.router_t2n_state = R_T2N_WAIT_CP_INIT;
...
/****************** router state machines - Transmit to Unknown *****/

//// to-do: can be merged with node- t2u_send_preamble_init
void iqueuemac_router_t2u_send_preamble_init(iqueuemac_t* iqueuemac){
	...
	// Here, the states fields are different from node-type, so maybe add a 'if' in the future when merging!!!.
	iqueuemac->router_states.router_t2u_state = R_T2U_SEND_PREAMBLE;
	iqueuemac->need_update = true;

	packet_queue_flush(&iqueuemac->rx.queue);
}

//// to-do: can be merged with node- t2u_send_preamble
void iqueuemac_router_t2u_send_preamble(iqueuemac_t* iqueuemac){
	...
	iqueuemac->router_states.router_t2u_state = R_T2U_WAIT_PREAMBLE_ACK;
	iqueuemac->need_update = false;
}

void iqueuemac_router_t2u_wait_preamble_ack(iqueuemac_t* iqueuemac){
	...
	if(iqueuemac->tx.got_preamble_ack == true){
		iqueuemac_clear_timeout(iqueuemac,TIMEOUT_PREAMBLE);
		iqueuemac_clear_timeout(iqueuemac,TIMEOUT_PREAMBLE_DURATION);
		iqueuemac->router_states.router_t2u_state = R_T2U_SEND_DATA;
		iqueuemac->need_update = true;
		return;
	}

	if(iqueuemac_timeout_is_expired(iqueuemac, TIMEOUT_PREAMBLE_DURATION)){
		iqueuemac->router_states.router_t2u_state = R_T2U_END;
		iqueuemac_clear_timeout(iqueuemac,TIMEOUT_PREAMBLE);
		iqueuemac->need_update = true;
		return;
	}
...
void iqueuemac_router_t2u_send_data(iqueuemac_t* iqueuemac){

	iqueuemac_send_data_packet(iqueuemac, NETOPT_DISABLE);

	iqueuemac->tx.tx_packet = NULL;

	iqueuemac->router_states.router_t2u_state = R_T2U_WAIT_TX_FEEDBACK;
	iqueuemac->need_update = true;
	/**** add a "wait-for-tx feedback" period to wait for the ending of the tx operation ***/
...
void iqueuemac_router_t2u_wait_tx_feedback(iqueuemac_t* iqueuemac){
...
void iqueuemac_router_t2u_update(iqueuemac_t* iqueuemac)
{
	switch(iqueuemac->router_states.router_t2u_state)
	{
	 case R_T2U_SEND_PREAMBLE_INIT: iqueuemac_router_t2u_send_preamble_init(iqueuemac);break;
     case R_T2U_SEND_PREAMBLE: iqueuemac_router_t2u_send_preamble(iqueuemac); break;
	 case R_T2U_WAIT_PREAMBLE_ACK: iqueuemac_router_t2u_wait_preamble_ack(iqueuemac); break;
	 case R_T2U_SEND_DATA: iqueuemac_router_t2u_send_data(iqueuemac); break;
	 case R_T2U_WAIT_TX_FEEDBACK: iqueuemac_router_t2u_wait_tx_feedback(iqueuemac);break;
	 case R_T2U_END: iqueuemac_router_t2u_end(iqueuemac);break;
	 default: break;
	}
}

/****************** router state machines - Transmit to router *****/

void iqueuemac_router_t2r_init(iqueuemac_t* iqueuemac){
	...
	iqueuemac->router_states.router_t2r_state = R_T2R_WAIT_CP;
	iqueuemac->need_update = true;
}
...
void iqueuemac_router_t2r_wait_cp(iqueuemac_t* iqueuemac){
	...
	if(iqueuemac->tx.current_neighbour->in_same_cluster == true){
		...
		if(iqueuemac->router_states.router_new_cycle == true){
			iqueuemac->router_states.router_new_cycle = false;
			iqueuemac_trun_on_radio(iqueuemac);
			iqueuemac->router_states.router_t2r_state = R_T2R_TRANS_IN_CP;
			/// should delay a small period of time here to ensure the destination has wake up!! add in the future!!
			//Router SHould Use smaller csma-backoff-exponent for entering channel!!!
			iqueuemac->need_update = true;
		}
	}else{
		if(iqueuemac_timeout_is_expired(iqueuemac, TIMEOUT_WAIT_CP)){
...
void iqueuemac_router_t2r_trans_in_cp(iqueuemac_t* iqueuemac){
	...
	iqueuemac->router_states.router_t2r_state = R_T2R_WAIT_CPTRANS_FEEDBACK;
	iqueuemac->need_update = true;

}

void iqueuemac_router_t2r_wait_cp_transfeedback(iqueuemac_t* iqueuemac){
...
void iqueuemac_router_t2r_end(iqueuemac_t* iqueuemac){
	...
	puts("Shuguo: router is in t-2-r end, switching back to sleeping period");
...
void iqueuemac_router_t2r_update(iqueuemac_t* iqueuemac)
{
	switch(iqueuemac->router_states.router_t2r_state)
	{
	 case R_T2R_WAIT_CP_INIT: iqueuemac_router_t2r_init(iqueuemac);break;
...
/*** router t-2-node modules ***/

void iqueuemac_router_t2n_init(iqueuemac_t* iqueuemac){
	...
	iqueuemac->router_states.router_t2n_state = R_T2N_WAIT_CP;
	iqueuemac->need_update = true;
}

void iqueuemac_router_t2n_wait_cp(iqueuemac_t* iqueuemac){
	...
	if(iqueuemac->tx.current_neighbour->in_same_cluster == true){
		...
		if(iqueuemac->router_states.router_new_cycle == true){
			iqueuemac->router_states.router_new_cycle = false;
			iqueuemac_trun_on_radio(iqueuemac);
			iqueuemac->router_states.router_t2n_state = R_T2N_TRANS_IN_CP;
			/// should delay a small period of time here to ensure the destination has wake up!! add in the future!!
			//Router SHould Use smaller csma-backoff-exponent for entering channel!!!
			iqueuemac->need_update = true;
		}
...
void iqueuemac_router_t2n_trans_in_cp(iqueuemac_t* iqueuemac){
	...
	iqueuemac->router_states.router_t2n_state = R_T2N_WAIT_CPTRANS_FEEDBACK;
	iqueuemac->need_update = true;
}

void iqueuemac_router_t2n_wait_cp_transfeedback(iqueuemac_t* iqueuemac){
	...
	if(iqueuemac->tx.tx_finished == true){
		...
		iqueuemac->router_states.router_t2n_state = R_T2N_TRANS_END;
		iqueuemac->need_update = true;
	}
}
...
void iqueuemac_router_t2n_end(iqueuemac_t* iqueuemac){
	...
	puts("Shuguo: router is in t-2-n end, switching back to sleeping period");
...
void iqueuemac_router_t2n_update(iqueuemac_t* iqueuemac)
{
	switch(iqueuemac->router_states.router_t2n_state)
	{
	 case R_T2N_WAIT_CP_INIT: iqueuemac_router_t2n_init(iqueuemac);break;
     case R_T2N_WAIT_CP: iqueuemac_router_t2n_wait_cp(iqueuemac); break;
	 case R_T2N_TRANS_IN_CP: iqueuemac_router_t2n_trans_in_cp(iqueuemac); break;
	 case R_T2N_WAIT_CPTRANS_FEEDBACK: iqueuemac_router_t2n_wait_cp_transfeedback(iqueuemac); break;
	 case R_T2N_TRANS_END:iqueuemac_router_t2n_end(iqueuemac);break;
	 default: break;
	}
}
