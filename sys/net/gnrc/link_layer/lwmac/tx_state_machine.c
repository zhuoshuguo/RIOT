...
void lwmac_tx_stop(gnrc_netdev2_t* gnrc_netdev2)
{
    ...
    lwmac_clear_timeout(gnrc_netdev2, TIMEOUT_NEXT_BROADCAST);
    lwmac_clear_timeout(gnrc_netdev2, TIMEOUT_BROADCAST_END);
    ...
    switch (gnrc_netdev2->tx.state) {
        case TX_STATE_INIT: {
            ...
            if (gnrc_netif_hdr_get_flag(gnrc_netdev2->tx.packet) &
               (GNRC_NETIF_HDR_FLAGS_BROADCAST | GNRC_NETIF_HDR_FLAGS_MULTICAST)) {
                ...
                gnrc_netdev2->dev->driver->set(gnrc_netdev2->dev, NETOPT_CSMA,
        ...
        case TX_STATE_SEND_BROADCAST: {
            ...
            if (lwmac_timeout_is_running(gnrc_netdev2, TIMEOUT_BROADCAST_END)) {
                if (lwmac_timeout_is_expired(gnrc_netdev2, TIMEOUT_BROADCAST_END)) {
                    lwmac_clear_timeout(gnrc_netdev2, TIMEOUT_NEXT_BROADCAST);
                ...
                gnrc_netdev2->dev->driver->set(gnrc_netdev2->dev, NETOPT_AUTOACK, &autoack,
                                               sizeof(autoack));
                first = true;
            }

            if (lwmac_timeout_is_expired(gnrc_netdev2, TIMEOUT_NEXT_BROADCAST) ||
                first) {
                ...
                lwmac_set_timeout(gnrc_netdev2, TIMEOUT_NEXT_BROADCAST,
        ...
        case TX_STATE_SEND_WR: {
            ...
            if ((gnrc_netdev2->tx.wr_sent == 0) &&
                (gnrc_netdev2_get_tx_continue(gnrc_netdev2) == false)) {
        ...
        case TX_STATE_WAIT_FOR_WA: {
            ...
            if (lwmac_timeout_is_expired(gnrc_netdev2, TIMEOUT_WR)) {
                if (gnrc_netdev2_get_tx_continue(gnrc_netdev2)) {
            ...
            while ((pkt = gnrc_priority_pktqueue_pop(&gnrc_netdev2->rx.queue)) != NULL) {
                ...
                if (memcmp(&info.src_addr.addr, &gnrc_netdev2->tx.current_neighbor->l2_addr,
                           gnrc_netdev2->tx.current_neighbor->l2_addr_len) == 0) {
                ...
                if (!(memcmp(&info.dst_addr.addr, &gnrc_netdev2->l2_addr,
                             gnrc_netdev2->l2_addr_len) == 0) && from_expected_destination) {
                    gnrc_mac_queue_tx_packet(&gnrc_netdev2->tx, 0, gnrc_netdev2->tx.packet);
                    /* drop pointer so it wont be free'd */
                    gnrc_netdev2->tx.packet = NULL;
                    postponed = true;
        ...
        case TX_STATE_SEND_DATA: {
            ...
            gnrc_netdev2->dev->driver->set(gnrc_netdev2->dev, NETOPT_AUTOACK,
